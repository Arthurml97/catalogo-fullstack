# Projeto: Catálogo de Produtos Full Stack (Spring Boot + jQuery)
...
1.  Navegue até a pasta `/catalogo-api`.
2.  Execute a aplicação Spring Boot (pela sua IDE ou via terminal com `mvn spring-boot:run`).
3.  O servidor estará rodando em `http://localhost:8080`.

### 2. Executando o Frontend (Cliente)

1.  Navegue até a pasta `/catalogo-cliente`.
2.  Abra o arquivo `index.html` diretamente no seu navegador.
3.  A interface irá carregar e se conectar automaticamente ao backend.
